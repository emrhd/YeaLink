...
<body>
  ...
  <div class="info">
    Telefonlarda kullanılacak rehber adresi (bu sayfa herkese açık olduğu için
    gerçek anahtarı burada GÖSTERMİYORUZ — Railway &gt; Variables &gt;
    PHONEBOOK_TOKEN değerini kendiniz alıp aşağıdaki YOUR_TOKEN yerine yazın):<br>
    <code>${req.protocol}://${req.get('host')}/phonebook.xml?key=YOUR_TOKEN</code>
    ${!PHONEBOOK_TOKEN ? '<br><br>⚠️ PHONEBOOK_TOKEN tanımlı değil — rehber şu an herkese açık. Railway Variables kısmından ekleyin.' : ''}
  </div>
</body>
</html>`);
});
